...
"""
def Cal(d, stair_list):
    for i in range(3, len(stair_list)-1):
        # n-1칸을 안 밟았을 때
        if d[i - 1] == d[i - 2]:
            d[i] = d[i - 1] + stair_list[i - 1]
        # n-1칸을 밟았을 때
            ...
            # n-2칸을 안 밟았을 때
            if d[i - 2] == d[i - 3]:
                if stair_list[i] + stair_list[i + 1] > stair_list[i - 1]:
                    d[i] = d[i - 1]
                else:
                    d[i] = d[i - 1] + stair_list[i - 1]
            # n-2칸을 밟았을 때
...
def Solution(stair_list):
    ...
    if len(stair_list) < 3:
        ...
        d[1] = stair_list[0]
        d[2] = stair_list[0] + stair_list[1]
        result_1 = Cal(d, stair_list)[-3]
        # OXO일 때
        d = [0 for _ in range(len(stair_list) + 1)]
        d[0] = 0
        d[1] = stair_list[0]
        d[2] = stair_list[0]
        result_2 = Cal(d, stair_list)[-3]
...
print(Solution(stair_list))
#고치다 망한 코드 2
#내 코드 살리면서 바꿔보려했는데 그냥 싸그리 엎고 새로 짜야겠다..
"""